...
<body>
  <div class="demo-content">
     <input id="leadNo" type="hidden" value="${lead_no}"/>
    <div class="row">
          ...
          <span id="oldPassSpan2" style="margin-left: 5px"></span> 
            ...
            <div class="row">
              <div class="form-actions offset3">
                <button type="submit" id="submit" class="button button-primary">保存</button>
...
<script type="text/javascript">
  $(function($){
	  var flag =0;
	 $("#newPassward1").focus(function(){
		 debugger
			$.ajax({
				type:"post",
				dataType:"json",
				data:{"lead_no":$("#leadNo").val(),"passward":$("#oldPassward").val()},
				url:"/check_LeaPassward",
	...
	$("#newPassward1").blur(function(){
		if(!Util.isBlank($("#newPassward1").val()) && $("#newPassward1").val().length>0){
	...
	$("#newPassward2").blur(function(){
		if(!Util.isBlank($("#newPassward2").val()) && $("#newPassward2").val().length>0){
			$("#oldPassSpan2").html("<i class='icon icon-ok icon-green' style='margin-top:5px;'></i>");
	 };
		if($("#newPassward1").val() != $("#newPassward2").val() && $("#newPassward1").val().length>0){
 	...
 	var  checkPass =function(){
		...
		$.ajax({
			type:"post",
			dataType:"json",
			data:{"lead_no":$("#leadNo").val().trim(),"passward":$("#oldPassward").val().trim()},
			url:"/check_LeaPassward",
			async:false,
			success:function(data){
				debugger
	 ...
	 $("#submit").click(function(){
		 ...
		 if(flag){
			 ...
			 $.ajax({
					type:"post",
					dataType:"json",
					data:{"lead_no":$("#leadNo").val(),"newPassward":$("#newPassward1").val()},
					url:"/lea_changePass",
					async:true,
					success:function(data){
						if(data.status == 0){
